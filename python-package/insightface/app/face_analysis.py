...
from extract_face_features import *
...
class FaceAnalysis:
    def __init__(self, name=DEFAULT_MP_NAME, root='~/.insightface', allowed_modules=None, **kwargs):
        ...
        if 'recognition' in self.models:
            self.face_database = read_pkl("face_database.pkl")
    ...
    def find_face(self,target_embedding,dis_thr=23):
        target_embedding = np.array(target_embedding)
        target_name = None
        min_dis = dis_thr
        for name in self.face_database.keys():
            for embedding in self.face_database[name]:
                dis = np.sqrt(np.square((np.array(embedding)-target_embedding)).sum())
                #print("embedding dis:" ,dis)
                if dis < min_dis:
                    min_dis = dis
                    target_name = name
        return target_name
    ...
    def draw_on(self, img, faces,face_dis_thr=23):
        ...
        for i in range(len(faces)):
            ...
            if name is not None:
                dimg = draw_text(dimg,name, (box[0] - 1, box[1] - 20),(0, 255, 0))
